      ...
      <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-4">
          <Link href={`/project/${projectId}`}>
            <button className="p-2 rounded-lg hover:bg-taxaidd-gray-light transition-colors">
              <ArrowLeft className="w-5 h-5 text-gray-600" />
            </button>
          </Link>
          <div>
            <h1 className="font-semibold text-taxaidd-black">{project.name}</h1>
        ...
        {/* Filter indicator */}
        {filteredDocumentIds && (
          <div className="flex items-center gap-2 bg-taxaidd-yellow/20 px-3 py-1.5 rounded-full">
            <span className="text-sm text-taxaidd-black">
              Mode filtré : {filteredDocumentIds.length} document{filteredDocumentIds.length > 1 ? 's' : ''} source
            </span>
            <button
              onClick={handleClearFilter}
              className="p-1 rounded-full hover:bg-taxaidd-yellow/30 transition-colors"
