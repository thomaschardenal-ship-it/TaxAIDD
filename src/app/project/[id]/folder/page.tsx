      ...
      <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-4">
          <Link href={`/project/${projectId}`}>
            <button className="p-2 rounded-lg hover:bg-wedd-cream transition-colors">
              <ArrowLeft className="w-5 h-5 text-gray-600" />
            </button>
          </Link>
          <div>
            <h1 className="font-semibold text-wedd-black">{project.name}</h1>
        ...
        {/* Filter indicator */}
        {filteredDocumentIds && (
          <div className="flex items-center gap-2 bg-wedd-mint/20 px-3 py-1.5 rounded-full">
            <span className="text-sm text-wedd-black">
              Mode filtré : {filteredDocumentIds.length} document{filteredDocumentIds.length > 1 ? 's' : ''} source
            </span>
            <button
              onClick={handleClearFilter}
              className="p-1 rounded-full hover:bg-wedd-mint/30 transition-colors"
